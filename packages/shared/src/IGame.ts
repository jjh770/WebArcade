/* ============================================================
   ...
   다섯 번째 게임(조준)이 `aim`을 요구했다. 방향 넷은 「어느 쪽으로 가는가」고 조준은
   「어디를 보는가」다 — 방향키로 조준점을 밀어 옮기게 만들 수도 있었지만, 그건 겨누는
   속도가 곧 점수인 게임을 방향키 반복 속도 재기로 바꿔 놓는다.
   ⚠️ InputState에 좌표를 얹지 **않은** 이유는 typeKey 때와 같다. 저건 방향 넷의
   눌림 여부이고, 여기에 좌표를 끼우면 좌표를 안 쓰는 게임 셋이 매 스텝 뜻 없는 숫자
   둘을 받는다. 쓰는 게임만 여는 문이라야 안 쓰는 게임이 그대로다.

   ⚠️ 선택 메서드는 여기까지다. 여섯 번째 게임이 실제로 요구하기 전에는 넓히지
...
export interface IGame {
  ...
  /** (선택) 조준점이 움직였다. 좌표는 **판 기준 0~1**이고, 판 밖은 가장자리에 붙어 온다.
   *  방향 넷으로는 표현되지 않는 둘째 입력이다(typeKey와 나란히 선다).
   *  ⚠️ 정규화 좌표인 이유: core는 게임 좌표계 크기를 모른다. 게임이 자기 세계 크기를
   *  곱해 쓴다(TouchMapper가 0~1을 받는 것과 같은 결).
   *  ⚠️ 고정 스텝 밖에서 온다. 로컬 입력이라 결정론과 무관하다 — 조준은 내 화면의
   *  일이고, 공통 월드는 시드가 정한 대로 남과 똑같이 흐른다. */
  aim?(nx: number, ny: number): void;

  /** (선택) 그 자리를 **쏘았다.** 좌표는 aim과 같은 규칙(판 기준 0~1)이고, 부르기 직전에
   *  aim이 같은 자리로 먼저 불린다 — 좌표를 안 받고 자기 조준점을 읽어도 맞는다.
   *  ⚠️ aim과 나눠 둔 이유: 좌표를 아무리 자주 보내도 **누른 순간**은 안 나온다.
   *  같은 자리에 서서 두 번 쏘는 것과 한 번 쏘는 것이 구별되지 않기 때문이다.
   *  ⚠️ 마우스는 왼쪽 버튼, 손가락은 톡 치기. 붙들고 있어도 한 번만 온다(연사가 아니다). */
  fire?(nx: number, ny: number): void;

  /** (선택) **시선을 이만큼 돌렸다**(판 크기 대비 비율). 마우스를 움직인 만큼 온다.
   *  ⚠️ `aim`과 배타적이다 — 저건 「어디를 겨누나」(절대 자리)고 이건 「얼마나 돌렸나」
   *  (상대 변화)다. 이 메서드를 구현한 게임은 **마우스에서 aim을 받지 않는다**:
   *  조준점이 화면 한가운데 붙박이고 판이 움직이는 FPS 방식이 되기 때문이다.
   *  ⚠️ **손가락은 여전히 aim으로 온다.** 폰에는 포인터 잠금이 없어 끌어서 돌리려면
   *  발사 버튼을 따로 둬야 하는데, 그건 「짚은 자리를 쏜다」를 뒤집는 일이라 안 했다
   *  (2026-08-18 사용자 결정). 그래서 같은 게임이 기기마다 다르게 조작된다. */
  look?(dnx: number, dny: number): void;

  /** (선택) **미리보기·배경이 겨눌 만한 자리**(판 기준 0~1). 겨눌 것이 없으면 null.
   *  ⚠️ 판정에 쓰이지 않는다 — 장식용이다. 목록의 작은 판과 메뉴 뒤 배경은 사람 없이
   *  혼자 도는데(gamePreview), 조준 게임은 그 자동 조작이 **표적을 못 찾는다.** 방향 넷은
   *  아무렇게나 움직여도 "누가 하고 있네"로 보이지만, 조준은 빗나가는 게 그대로 보여서
   *  40발을 쏘고 한 발도 못 맞히는 화면이 된다(실제로 그랬다).
   *  ⚠️ 그렇다고 미리보기가 게임 규칙을 알게 할 수는 없다. 그래서 **어디를 겨눌지는 게임이
   *  말하고** 언제 쏠지는 미리보기가 정한다. 조준을 안 쓰는 게임은 구현하지 않는다. */
