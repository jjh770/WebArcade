...
export const jungnimConfig = {
  ...
  item: {
    ...
    kinds: [
      { kind: "dash", weight: 40, color: "#4dd0e1", label: "질주" },
      { kind: "shield", weight: 25, color: "#a8dadc", label: "쉴드" },
      { kind: "focus", weight: 20, color: "#c77dff", label: "조준 정지" },
      { kind: "purge", weight: 15, color: "#ffd166", label: "정화" },
    ],

    /** 질주: 이동속도 배수. 다음 아이템까지 닿게 해줘 아이템끼리 연결된다. 300 = 5초. */
    dash: { durationTicks: 300, speedMult: 1.6 },
    /** 쉴드: 맞아도 이 횟수만큼 버틴다. 막은 화살은 부서진다 — 안 부수면 겹친 채로
     *  다음 tick에 또 맞아 3회가 한순간에 날아간다. */
    shield: { charges: 3 },
     ...
     *  남아 있다(개인 화살이 근사인 것과 같은 종류의 오차). 가장 희귀한 구제기라 감수한다.
