/* ============================================================
   ...
   이 파일은 **매 tick 도는 일**만 맡는다 — 주행·충돌·스침 게이지·렌더. 판을
   짓는 일(장애물·시작 자리·출발 방향)은 init에서 한 번만 돌고 끝나므로
   worldGen.ts에, 도형과 판정은 obstacles.ts에 있다.
...
import { HEAD_R, SEG_HALF, drawObstacle, obstacleBlocks, obstacleClearance, type Obstacle } from "./obstacles";
import { chooseOpenHeading, generateObstacles, pickSpawn } from "./worldGen";
...
export class CurveGame implements IGame {
  ...
  init(seed: number, self?: SpawnContext): void {
    ...
    // ⚠️ 이 세 줄의 순서가 곧 판의 정체성이다 — 한 시드에서 뽑는 순서를 바꾸면
    //    같은 시드가 다른 판이 된다. 이유는 worldGen.ts 머리말 참조.
    this.obstacles = generateObstacles(rng);
    const spawn = pickSpawn(rng, this.obstacles, self);
    this.x = spawn.x;
    this.y = spawn.y;
    this.angle = chooseOpenHeading(rng, this.obstacles, spawn.x, spawn.y);
