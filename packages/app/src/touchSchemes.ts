/* ============================================================
   ...
   - keypad         : 애초에 **방향이 아니다** → InputState를 만들지 않는다.
   그래서 SchemeSpec은 판별 유니온이다. buttons에 map을 다는 실수가 타입에서 막힌다.

   ⚠️ keypad는 앞의 셋과 종류가 다르다. 저 셋은 InputState(누르고 있는 방향)를 만들지만
      이건 글자 슬러그를 **밀어 넣는다**(IGame.typeKey). 조작면·세로 예산이라는 점만
      같아서 여기 함께 두고, 입력이 흐르는 길은 따로다(touchControls의 onKey).
   ============================================================ */

import { joystick8, splitLeftRight, type TouchMapper } from "@arcade/core";

export type TouchScheme = "halves" | "joystick" | "buttons" | "keypad";
