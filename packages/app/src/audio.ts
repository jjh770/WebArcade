...
const SOUNDS: Record<SoundId, Tone> = {
  ...
  // "탕"이 안 난다. crack과 같은 길(화이트 노이즈 + 밴드패스 훑기)을 쓴다.
  // ⚠️ 처음에는 7000Hz에서 시작해 0.07초로 끊었더니 «탁» 하고 얇았다(2026-08-17 사용자:
  //    "좀 묵직하게"). 시작을 1400Hz로 낮추고 55Hz까지 끌고 내려가며 길이를 두 배로
  //    늘렸다 — 높은 데서 시작하면 총성이 아니라 나뭇가지 부러지는 소리가 된다.
  // ⚠️ 맞든 안 맞든 **쏠 때마다** 난다. 헛방에 따로 소리를 두지 않는 이유가 이것이다 —
  //    총성만 나고 아무 반응이 없는 것이 곧 빗나감이고, 난사할 때 둔탁한 소리가
  //    겹겹이 울리지도 않는다.
  shot: { wave: "square", freq: [1400, 55], step: 0.14, gain: 0.6, noise: true },
