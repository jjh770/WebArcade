/* ============================================================
   Room — 방 상태 (packages/server/src/Room.ts에서 이식)
   ------------------------------------------------------------
   게임 내용을 모르는 순수 로직이라 거의 그대로 옮겼다.
   ⚠️ 이식 중에는 원본(packages/server)도 살아 있어 **같은 로직이 두 벌** 있다.
      한쪽만 고치면 조용히 갈라진다. 이식이 끝나면 원본을 지운다.

   추가된 것: `snapshot()` / `restore()`.
   Durable Object는 하이버네이션되면 메모리 필드가 날아가므로, 방 상태를
   ctx.storage에 넣었다 되살릴 수 있어야 한다. 클래스 인스턴스는 저장할 수
   없으니 평범한 객체로 바꿨다 되돌리는 통로가 필요하다.
   ============================================================ */
