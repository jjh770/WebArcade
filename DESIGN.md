...
사라진다. 방 코드로 `idFromName(code)` 하면 어느 지역에서 접속하든 같은 오브젝트로 라우팅되기
때문이다. 무료 티어 조건은 확인해 뒀다(들어오는 WebSocket 메시지에 20:1 청구 비율이 적용되어
10명 기준 하루 5.5시간). **호스팅 크레딧이 소진되어 이식을 시작했다** — `packages/edge/`.

#### 대가: 접속 시점에 방 코드를 알아야 한다

공짜가 아니다. **WebSocket이 어느 오브젝트로 갈지는 접속 URL로 정해지고, 한번 붙은 소켓은
다른 오브젝트로 옮길 수 없다.** 그런데 기존 흐름은 "일단 접속 → `create_room` 메시지로 방
생성"이라, 방이 생기기 전에는 갈 곳이 정해지지 않아 이 순서가 성립하지 않는다.

그래서 방 만들기가 **`POST /rooms`로 코드를 먼저 받고 그 코드로 접속**하는 2단계로 바뀐다.
`NetClient`도 여기 맞춰 바뀌어야 한다 — 이식 초기에 "클라는 안 바뀐다"고 봤던 것은 **틀렸다.**
전송 계층이 투명하다는 가정이 깨지는 지점이다.

Worker가 소켓을 받아 DO로 중계하면 클라를 안 바꿀 수는 있지만, 그러면 Worker가 연결 내내
살아 있어야 해서 하이버네이션의 이점(유휴 시 무과금)이 사라진다. 클라를 바꾸는 쪽이 싸다.
...
하며, 이게 "깨어난 직후 빈 상태로 요청을 받는" 사고를 막는 유일한 장치다.
