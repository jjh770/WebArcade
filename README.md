...
(`core`는 한 줄도 안 바뀌었고, `IGame`은 선택 메서드 3개만 넓혔다).
...
`getGauge()`(HUD 게이지 줄) · `consumePendingFire()`(방해 발사) · `applyEffect(kind, ms)`(피격).
두 게임 다 「스릴 게이지」로 이걸 쓰지만 충전 방식은 서로 다르다(아래 참조).
...
## 스릴 게이지 — 유일한 플레이어 간 간섭

아슬아슬하게 피할수록 게이지가 차고, 꽉 차면 **지금 내 우측 관전창에 떠 있는 상대 1명**에게
방해 디버프를 쏜다. 탄환이 그 관전창까지 날아가 명중하는 연출이 붙는다.

| | 죽림고수 | 커브 피버 |
|---|---|---|
| 충전 | 화살이 피격 반경 바로 밖(16px)을 스침 = 1회, **10회**면 발사 | 벽·꼬리·장애물 스침 정도에 비례해 연속 충전, **100%** 면 발사 |
| `invert` | 상하좌우 4방향 반전 | 좌/우 반전 |
| `sluggish` | 이동 속도 ×0.45 | 회전 속도 ×0.4 |

`blur`·`shake`·`cloud`는 앱이 victim 화면에 DOM/CSS로 걸어 게임과 무관하다.

> **왜 결정론이 안 깨지나**: 효과는 맞은 사람의 *입력·시야*만 바꾼다. 공통 월드(시드 기반)는
> 전혀 안 건드리므로 클라 간 월드가 어긋나지 않는다 — 락스텝 입력 동기가 필요 없다.
